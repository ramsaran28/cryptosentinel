...
async function runPipeline(coinId) {
  const res = await fetch("/mastra/workflows/cryptoPipeline/start-async", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ inputData: { coin: coinId } }),
  });
  ...
  return {
    anomalyReport: data?.steps?.["detect-anomaly"]?.output?.anomalyReport
      || data?.result?.anomalyReport
      || "No anomaly detected.",
    explanation: data?.result?.explanation
      || data?.steps?.["explain-alert"]?.output?.explanation
      || "Analysis complete.",
  };
}
...
function CandleChart({ candles, currentPrice }) {
  ...
  useEffect(() => {
    ...
    const candleW = Math.max(4, gap - 2);
  ...
  return (
    <canvas ref={canvasRef} width={900} height={360} style={{ width: "100%", height: "100%" }} />
  );
}
...
export default function CryptoDashboard() {
  ...
  const [anomaly, setAnomaly] = useState("Loading anomaly data from Agent 2...");
  const [aiInsight, setAiInsight] = useState("Loading AI explanation from Agent 4...");
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setAnomaly("⏳ Running Agent 2...");
      setAiInsight("⏳ Running Agent 4...");
      try {
        const result = await runPipeline(activeCoin.id);
        setAnomaly(result.anomalyReport);
        setAiInsight(result.explanation);
        setPipelineStatus("done");
      } catch (e) {
        console.error("Pipeline error:", e);
        setAnomaly("Error: " + e.message);
  ...
  useEffect(() => {
    ...
    fetch(`/coingecko/api/v3/coins/markets?vs_currency=usd&ids=${activeCoin.id}&price_change_percentage=24h`)
      .then((r) => r.json())
      .then((d) => { if (d[0]) { setPrice(d[0].current_price); setChange24h(d[0].price_change_percentage_24h); } })
      .catch(() => {});

    fetch(`/binance-api/api/v3/klines?symbol=${activeCoin.wsSymbol}&interval=1m&limit=60`)
      .then((r) => r.json())
      .then((data) => {
        const historical = data.map((k) => ({
          time: k[0], open: parseFloat(k[1]), high: parseFloat(k[2]),
          low: parseFloat(k[3]), close: parseFloat(k[4]), volume: parseFloat(k[5]),
        }));
        historical.forEach((c) => { candleRef.current[c.time] = c; });
        setCandles([...historical]);
      })
      .catch(() => {});

    const ws = new WebSocket(`ws://localhost:5176/binance-ws/ws/${activeCoin.wsSymbol.toLowerCase()}@kline_1m`);
    wsRef.current = ws;
  ...
  return (
    <div style={{ background: "#0a0a0f", minHeight: "100vh", fontFamily: "'JetBrains Mono', 'Courier New', monospace", color: "#e2e8f0", padding: 0 }}>
      <div style={{ background: "#0d0d15", borderBottom: "1px solid #1a1f2e", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {COINS.map((c) => (
            <button key={c.id} onClick={() => setActiveCoin(c)} style={{
              ...
              padding: "6px 16px", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 700, letterSpacing: 1, fontFamily: "inherit",
            }}>{c.symbol}</button>
          ))}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: pipelineStatus === "running" ? "#ffd60a" : pipelineStatus === "done" ? "#00d97e" : "#ff4d6d" }} />
          <span style={{ fontSize: 11, color: "#4a5568" }}>
            {pipelineStatus === "running" ? "PIPELINE RUNNING..." : pipelineStatus === "done" ? "AGENTS LIVE" : connected ? "LIVE" : "CONNECTING"}
          </span>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 0, height: "calc(100vh - 53px)" }}>
        <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 16 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: isUp ? "#00d97e" : "#ff4d6d", background: isUp ? "#00d97e15" : "#ff4d6d15", padding: "2px 10px", borderRadius: 4 }}>
              {isUp ? "+" : ""}{change24h?.toFixed(2)}%
            </span>
            <span style={{ fontSize: 11, color: "#4a5568" }}>{activeCoin.symbol}/USDT • 1m</span>
          </div>

          <div style={{ background: "#0d0d15", border: "1px solid #1a1f2e", borderRadius: 8, flex: 1, overflow: "hidden" }}>
            {candles.length === 0 ? (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "#4a5568", fontSize: 12 }}>Waiting for live data...</div>
        ...
        <div style={{ borderLeft: "1px solid #1a1f2e", background: "#0d0d15", display: "flex", flexDirection: "column", overflowY: "auto" }}>
          <div style={{ padding: 16, borderBottom: "1px solid #1a1f2e" }}>
          ...
          <div style={{ padding: 16, borderBottom: "1px solid #1a1f2e" }}>
            <div style={{ fontSize: 9, color: "#4a5568", letterSpacing: 2, marginBottom: 10 }}>ANOMALY DETECTOR — AGENT 2</div>
            <div style={{
              background: anomaly.includes("ALERT") ? "#ff4d6d15" : "#00d97e08",
              border: `1px solid ${anomaly.includes("ALERT") ? "#ff4d6d40" : "#00d97e20"}`,
              borderRadius: 6, padding: "10px 12px", fontSize: 11,
              color: anomaly.includes("ALERT") ? "#ff4d6d" : "#00d97e",
              lineHeight: 1.6,
            }}>{anomaly}</div>
          </div>

          <div style={{ padding: 16, borderBottom: "1px solid #1a1f2e" }}>
            <div style={{ fontSize: 9, color: "#4a5568", letterSpacing: 2, marginBottom: 10 }}>AI INSIGHT — AGENT 4</div>
            <div style={{ background: "#a78bfa10", border: "1px solid #a78bfa30", borderRadius: 6, padding: "10px 12px", fontSize: 11, color: "#c4b5fd", lineHeight: 1.7 }}>
              {aiInsight}
            </div>
          </div>

          <div style={{ padding: 16 }}>
            <div style={{ fontSize: 9, color: "#4a5568", letterSpacing: 2, marginBottom: 10 }}>MARKET STATUS</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ background: "#0a0a0f", border: "1px solid #1a1f2e", borderRadius: 6, padding: "10px 12px", fontSize: 11, color: "#e2e8f0" }}>
                Fear & Greed: <span style={{ color: fearGreed.value < 50 ? "#ff4d6d" : "#00d97e", fontWeight: 700 }}>{fearGreed.value} — {fearGreed.label}</span>
              </div>
              <div style={{ background: "#0a0a0f", border: "1px solid #1a1f2e", borderRadius: 6, padding: "10px 12px", fontSize: 11, color: "#4a5568" }}>
                Pipeline refreshes every 5 minutes
              </div>
            </div>
          </div>
        </div>
